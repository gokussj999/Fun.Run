...
export interface BuyExecutionOpts {
  /** Caller-generated key — must be unique per logical buy attempt. */
  idempotencyKey:    string;
  /** Buyer's custodial wallet public key (base-58). */
  ...
  /** SPL token mint for the coin being bought. */
  mint:              PublicKey;
  /** SOL amount to spend, in lamports. */
  solAmount:         bigint;
  /** Minimum tokens to receive (slippage guard). */
  minTokensOut:      bigint;
...
/**
 * BuyExecutor — drives a buy trade through the full pre-confirmation lifecycle:
 *
...
export class BuyExecutor {
  ...
  async execute(opts: BuyExecutionOpts): Promise<BuyExecutionResult> {
    ...
    if (existing !== null) {
      if (existing.signature !== null) {
        this.logger.info(
          ...
          'BuyExecutor: idempotent — returning existing signature',
        );
        return { txId: existing.id, signature: existing.signature, idempotent: true };
      }
      // No signature yet — another request is in flight for this key.
    ...
    this.logger.info(
      {
        ...
        minTokensOut:  opts.minTokensOut.toString(),
      },
      'BuyExecutor: started',
    );
    ...
    try {
      ...
      const instruction = this.program.buy({
        buyer:        walletPubkey,
        mint:         opts.mint,
      ...
      this.logger.debug(
        ...
        'BuyExecutor: blockhash + fee ready',
      );
      ...
      const built = await this.txBuilder.build({
        ...
        blockhash:               bh,
        computeUnitLimit:        CU_BUDGETS.BUY,
      ...
      this.logger.debug(
        ...
        'BuyExecutor: tx built and simulated',
      );
      ...
      this.logger.debug({ txId, signer: walletPubkey.toBase58() }, 'BuyExecutor: signed');
      ...
      this.logger.info({ txId, signature }, 'BuyExecutor: submitted — awaiting confirmation');
  ...
  private async markFailed(txId: string, err: unknown): Promise<void> {
    ...
    this.logger.error(
      ...
      'BuyExecutor: execution failed — marking FAILED',
    );
    try {
      await this.machine.toFailed(txId, message, code, canResubmit);
    } catch (markErr) {
      // Swallow — do not mask the original error.
      this.logger.warn({ txId, err: markErr }, 'BuyExecutor: could not mark tx FAILED (already transitioned)');
    }
  }
}
